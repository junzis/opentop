...
def interp():
    return replay.build_contrail_interpolant(_fake_meteo_df(), sigma=2)


def test_build_contrail_interpolant_returns_casadi_function(interp):
    assert isinstance(interp, ca.Function)
    assert interp.n_in() == 1
    assert interp.sparsity_in(0).size1() == 4


def test_build_contrail_interpolant_finite_at_sample_point(interp):
